    ...
    <div className="flex flex-col gap-6 font-sans">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 dark:text-slate-500 mt-0.5">
          Consulta, filtra y gestiona el historial de todas tus solicitudes
      ...
      <div className="flex border-b border-slate-100 dark:border-slate-800 text-xs font-semibold gap-2 overflow-x-auto pb-0.5">
        {(
          ...
          const count =
            tab === "TODAS"
              ? solList.length
              : solList.filter((s) => getStatusDisplay(s.estado).group === tab)
                  .length;
      ...
      {loading ? (
        <div className="flex flex-col gap-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-24 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl animate-pulse"
            />
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-10 text-center flex flex-col items-center justify-center">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-550 mt-1">
            Prueba cambiando los filtros o realiza una solicitud en la página de
            Inicio.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex flex-col gap-2 flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 flex flex-col gap-1 mt-1">
                    ...
                    <p>
                      <strong>Destino:</strong> Lat:{" "}
                      {s.ubicacion_destino.coordinates[1].toFixed(6)}, Lng:{" "}
                      {s.ubicacion_destino.coordinates[0].toFixed(6)}
                    </p>
                    {s.id_base && (
                      <p>
                        <strong>Base asignada:</strong> Base #
                        {s.id_base.substring(0, 8).toUpperCase()}
                      </p>
                    )}
                    {s.motivoCancelacion && (
                ...
                <div className="flex flex-col gap-1 border-t border-b border-slate-50 dark:border-slate-800 py-3 md:border-0 md:py-0 md:px-6 max-w-xs text-xs">
                  <span className="font-semibold text-slate-400 dark:text-slate-500 block mb-1">
                    Productos:
                  </span>
                  {s.productos &&
                    s.productos.map((p: any, idx: number) => (
                      ...
                      >
                        <span>
                          •{" "}
                          {p.productoId === "Vacunas y Suero Fisiológico"
                            ? "Vacunas"
                            : p.productoId === "Botiquín de Primeros Auxilios"
                              ? "Botiquín"
                              : "Raciones"}
                        </span>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden p-6 flex flex-col gap-5">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                ...
                {solicToCancel.id.substring(0, 8).toUpperCase()}? Esta acción
