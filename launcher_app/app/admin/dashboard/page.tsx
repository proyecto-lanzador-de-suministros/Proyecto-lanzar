...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchSolicitudes = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error("Error al obtener las solicitudes. Verifica tu conexión o sesión.");
        }
  ...
  const getStatusColor = (estado: string) => {
    switch (estado) {
      ...
      case 'Aprobada': return 'bg-indigo-100 text-indigo-800 border-indigo-200';
      case 'Asignada': return 'bg-teal-100 text-teal-800 border-teal-200';
      case 'En preparación': return 'bg-orange-100 text-orange-800 border-orange-200';
      case 'Listo': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
      case 'En Camino': return 'bg-purple-100 text-purple-800 border-purple-200';
      case 'Lanzada': return 'bg-pink-100 text-pink-800 border-pink-200';
    ...
    <div className="min-h-screen bg-[#f4f7f6] p-6 md:p-10 font-sans text-slate-800">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-[#1e293b] tracking-tight">Panel de Control</h1>
          ...
          <div className="bg-white p-2 rounded-lg shadow-sm border border-gray-200 flex items-center">
            <span className="text-gray-500 pl-2 text-sm font-medium mr-2">Filtrar:</span>
            <select 
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          {loading ? (
            <div className="p-16 flex justify-center items-center">
              ...
              <span className="ml-4 text-gray-500 font-medium">Cargando datos...</span>
            </div>
          ) : error ? (
            <div className="p-16 text-center text-red-500 bg-red-50 font-medium">{error}</div>
          ) : solicitudes.length === 0 ? (
            <div className="p-16 text-center text-gray-500">
              <p className="text-lg font-medium text-gray-700">No hay solicitudes para mostrar</p>
              <p className="text-sm mt-1">Intenta cambiando el filtro de búsqueda.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 text-sm">
                  {solicitudes.map((solicitud) => (
                    <tr key={solicitud.id} className="hover:bg-[#fff7f0] transition-colors group">
                      <td className="p-5 font-mono text-xs text-gray-500">
                        #{solicitud.id.substring(0, 8)}
                      </td>
                      <td className="p-5 text-gray-700 font-medium">
                        {new Date(solicitud.fechaSolicitada).toLocaleDateString('es-AR', {
                      ...
                      <td className="p-5">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(solicitud.estado)}`}>
                          {solicitud.estado}
                        </span>
                      </td>
